...
Write-Host "=== 1/4: uv sync (raíz del repo) ===" -ForegroundColor Cyan
Push-Location $RepoRoot
try {
    uv sync
    if ($LASTEXITCODE -ne 0) { throw "uv sync falló (código $LASTEXITCODE)" }
} finally {
    Pop-Location
}

Write-Host "=== 2/4: fetch_models.py ===" -ForegroundColor Cyan
if ((Test-Path $ModelsDir) -and -not $RefetchModels) {
    Write-Host "models/ ya existe — se salta la descarga (usa -RefetchModels para forzarla)."
} else {
    Push-Location $RepoRoot
    try {
        uv run python scripts/fetch_models.py
        if ($LASTEXITCODE -ne 0) { throw "fetch_models.py falló (código $LASTEXITCODE)" }
    } finally {
        Pop-Location
    }
}
...
if (-not (Test-Path (Join-Path $DistDir 'server_main.exe'))) {
    throw "El build terminó pero no se encontró $DistDir\server_main.exe — revisar el log de pyinstaller arriba."
}
...
if ($SkipScrub) {
    ...
    Write-Host "Scrub completo — $DistDir está lista para zippear y distribuir." -ForegroundColor Green
}
...
Write-Host "reales) a esa carpeta — el .env del smoke test fue borrado a propósito arriba."
Write-Host ""
Write-Host "Antes de REDISTRIBUIR una actualización a un concesionario EXISTENTE: preservar"
...
Write-Host "porque no tiene acceso a la máquina del concesionario."
